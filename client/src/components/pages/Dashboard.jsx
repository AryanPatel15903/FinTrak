  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        // Set the pending expenses count
        setPendingCount(pendingResponse.data.count);

        // Fetch recent expenses (last 5 expenses)
        const recentResponse = await axios.get('http://localhost:8080/api/expenses/recent', {
          headers: { 'x-auth-token': token },
        });

        // Set the recent expenses
        setRecentExpenses(recentResponse.data);

      } catch (err) {
        console.error('Failed to fetch user data or pending count', err);
        setIsLoggedIn(false);
      }
    };
