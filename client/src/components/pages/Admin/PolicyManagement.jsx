...
export default function PolicyManagement() {
  ...
  const updateStats = (policyData) => {
    const uniqueCategories = new Set(policyData.map(policy => policy.category)).size;
    const totalLimit = policyData.reduce((sum, policy) => sum + (parseInt(policy.limit) || 0), 0);
    const avgLimit = policyData.length ? Math.round(totalLimit / policyData.length) : 0;
    ...
    <div className="space-y-8 p-6 bg-gray-50 min-h-screen">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
        <div>
          ...
          <p className="text-gray-600 mt-1">Create and manage expense policies for your organization</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-lg shadow-md border-l-4 border-blue-500">
          <h2 className="text-lg font-medium text-gray-600 mb-2">Total Policies</h2>
          <div className="flex items-center">
            <span className="text-3xl font-bold text-blue-600">{stats.totalPolicies}</span>
        ...
        <div className="bg-white p-6 rounded-lg shadow-md border-l-4 border-green-500">
          <h2 className="text-lg font-medium text-gray-600 mb-2">Average Limit</h2>
          <div className="flex items-center">
            <span className="text-3xl font-bold text-green-600">${stats.averageLimit}</span>
            <svg className="h-8 w-8 ml-auto text-green-500 opacity-75" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div className="bg-white p-6 rounded-lg shadow-md border-l-4 border-purple-500">
          ...
          <div className="flex items-center">
            <span className="text-3xl font-bold text-purple-600">{stats.categories}</span>
            <svg className="h-8 w-8 ml-auto text-purple-500 opacity-75" fill="currentColor" viewBox="0 0 20 20">
      ...
      <div className="bg-white rounded-lg shadow-md">
        <div className="p-6">
          ...
          <form onSubmit={editingPolicy ? handleEditPolicy : handleAddPolicy} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <select
                  id="category"
                  name="category"
                  value={editingPolicy ? editingPolicy.category : newPolicy.category}
                  onChange={(e) => {
                    if (editingPolicy) {
                      setEditingPolicy({...editingPolicy, category: e.target.value});
                    } else {
                      handleChange(e);
                    }
                  }}
              ...
              <div>
                ...
                <div className="relative mt-1">
                  ...
                  <input
                    ...
                    id="limit"
                    name="limit"
                    value={editingPolicy ? editingPolicy.limit : newPolicy.limit}
                    onChange={(e) => {
                      if (editingPolicy) {
                        setEditingPolicy({...editingPolicy, limit: e.target.value});
                      } else {
                        handleChange(e);
                      }
                    }}
                    required
                    min="0"
                    className="w-full p-2 pl-8 rounded-md border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    placeholder="0"
                    disabled={submitting}
                  />
                </div>
            ...
            <div className="overflow-x-auto">
              {filteredPolicies.length > 0 ? (
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {filteredPolicies.map((policy) => (
                      <tr key={policy._id} className="hover:bg-gray-50">
                          ...
                          <div className="flex items-center">
                            <div className="h-10 w-10 flex-shrink-0 mr-3">
                              <div className={`h-10 w-10 rounded-full flex items-center justify-center 
                                ${policy.category === 'travel' ? 'bg-blue-100 text-blue-600' : 
                                  policy.category === 'meals' ? 'bg-green-100 text-green-600' : 
                                  policy.category === 'office' ? 'bg-purple-100 text-purple-600' : 
                                  policy.category === 'training' ? 'bg-yellow-100 text-yellow-600' : 
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-gray-900">${policy.limit}</div>
                        </td>
                ...
                <div className="bg-gray-50 rounded-lg py-12 text-center">
                  ...
                  <p className="mt-1 text-sm text-gray-500">Get started by creating your first policy using the form above.</p>
                </div>
              )}
            </div>
          )}
