...
export default function MyExpenses() {
  ...
  const categories = ["Travel", "Meals", "Office Supplies", "Equipment", "Transportation", "Other"];
  ...
  const filteredExpenses = expenses.filter(expense => {
    ...
    if (filters.dateRange.startDate && new Date(expense.date) < new Date(filters.dateRange.startDate)) {
      return false;
    }
    if (filters.dateRange.endDate && new Date(expense.date) > new Date(filters.dateRange.endDate)) {
      return false;
    }
    ...
    <div className="max-w-6xl mx-auto mt-6 px-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-white p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-semibold mb-2 text-gray-700">Rejected</h2>
      ...
      <div className="flex justify-between items-center mb-4">
        <div className="text-gray-600">
          Showing {filteredExpenses.length === 0 ? 0 : indexOfFirstExpense + 1} to {Math.min(indexOfLastExpense, filteredExpenses.length)} of {filteredExpenses.length} expenses
        </div>
      </div>
      ...
      {filteredExpenses.length === 0 ? (
        <div className="bg-white p-8 rounded-lg shadow-md text-center">
          ...
          <p className="text-gray-500 mt-1">Try adjusting your filters or add new expenses</p>
        </div>
      ) : (
              ...
              <tbody className="divide-y divide-gray-200">
                {currentExpenses.map((expense) => (
                  <tr key={expense._id} className="hover:bg-gray-50 transition-colors duration-150">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{expense.category_id}</td>
      ...
      {filteredExpenses.length > expensesPerPage && (
        <div className="flex justify-center mt-6">
          <nav className="inline-flex rounded-md shadow-sm">
            ...
            {/* Page Numbers */}
            {[...Array(Math.ceil(filteredExpenses.length / expensesPerPage)).keys()].map(number => (
            ...
            <button
              onClick={() => paginate(currentPage + 1)}
              disabled={currentPage === Math.ceil(filteredExpenses.length / expensesPerPage)}
              className={`relative inline-flex items-center px-4 py-2 rounded-r-md border ${
                currentPage === Math.ceil(filteredExpenses.length / expensesPerPage)
                  ? 'bg-gray-100 text-gray-400 cursor-not-allowed'
                  : 'bg-white text-gray-700 hover:bg-gray-50'
              } text-sm font-medium`}
            >
