...
router.post("/submit", auth, async (req, res) => {
  ...
  try {
    ...
    const remainingBudget = user.remainingBudget || 0;
    if (remainingBudget < amount) {
      return res.status(400).json({
        message: `Expense exceeds the remaining budget. Remaining: ${remainingBudget}`,
      });
    }

    // Deduct the expense amount from the user's total budget
    user.remainingBudget = remainingBudget - amount; // Update the remaining budget field
    await user.save();
    ...
    res
      .status(201)
      .json({
        message: "Expense submitted and email sent successfully",
        data: savedExpense,
      });
  } catch (error) {
